...
export default function SignupPage() {
  ...
  const handleSignup = async () => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (newUser) => {
      if (newUser) {
        unsubscribe(); // Unsubscribe to prevent this from running again on subsequent auth changes
        ...
        const userData = {
            ...
            address,
            isAdmin: false,
            document: accountType === 'pf' ? cpf : cnpj, // Consolidated document field
            ...(accountType === 'pf' && { birthDate: date }),
            ...(accountType === 'pj' && { companyName, tradingName, cnpj }),
        };
        
        // Use setDoc which returns a promise
        setDoc(userRef, userData, { merge: true }).then(() => {
            ...
            toast({
                ...
                description: "Sua conta foi criada, mas não foi possível salvar seus dados. Por favor, contate o suporte."
            });
        });
      }
    });

    initiateEmailSignUp(auth, email, password, handleSignupError);
  };
